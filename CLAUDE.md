...
**의존성은 늘리지 않는다.** OS 바인딩과 암호 원시함수는 예외. 다만 **어쩔 수 없이
필요하다면 복잡도/의존성을 늘려도 된다** — 기능의 한계를 코드로 우회할 수 없고, 그
한계가 사용자에게 실질적 손해(예: 음질)를 강요할 때. 이번 사례: AAC보다 Opus가 음질이
낫지만 iOS libVLC의 오디오 출력 한계로 Opus를 워치 운동 중 깨끗이 재생할 수 없었다 —
이럴 땐 플레이어에 기능(예: 자체 버퍼 관리/오디오 출력)을 추가 구현해서라도 사용자가
음질을 포기하지 않게 한다. 편의를 위한 의존성 추가는 여전히 금지; "다른 방법이 없을 때"만.
