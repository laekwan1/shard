...
final class WebModel: NSObject, ObservableObject, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    // 웹 잠금화면 Now Playing 패널은 WKWebView가 별도 미디어 프로세스에서 자체 관리해 앱에서 못 없앤다 —
    // 오디오 세션(.mixWithOthers/.soloAmbient/Now Playing 비우기)도, 웹페이지 mediaSession 억제도 무효였다
    // (소리만 죽거나 그대로). 사파리 패널을 앱에서 못 없애는 것과 같은 플랫폼 한계. 그래서 패널 제거는 접고,
    // 소리만 백그라운드 설정에 맞춘다(BrowserScreen pauseWebVideos). 자세한 경위는 docs/결함-기록.md.
