...
// 소유 JSON {"ok":true,"path":"..."} 또는 {"ok":false,"error":"..."}; shard_string_free. 목록은 log로.
char *shard_rsd_probe(const char *addr, uint16_t port, const char *pairing_path,
                      ShardLog log, void *ctx);

// ④ RSD 설치(iOS 17+): rppairing 터널 위에서 서명된 .ipa를 업로드(AFC)+설치(installation_proxy).
// pairing_path=RpPairingFile, ipa_path=⑤ 재서명 결과. 같은 번들ID면 in-place 업그레이드(데이터 보존).
// 소유 JSON 반환; shard_string_free.
char *shard_rsd_install(const char *addr, uint16_t port, const char *pairing_path,
                        const char *ipa_path, ShardLog log, void *ctx);
